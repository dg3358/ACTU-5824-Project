...
```
Statistically significant lags up to lag 4. I run an ar model, with order.max at 10, to check for stationarity:
...
ccf(diff(train_data$Spread),
    diff(train_data$Yield),
    lag.max = 200, plot = FALSE)[seq(-50, -25, 1)]
...
```{r, include = TRUE}
spread_lag_36 <- diff(full_data$Spread)[seq(1, length(train_data$Spread) - 37)]
spread_lag_36_test <- diff(full_data$Spread)[seq(length(train_data$Spread) - 36,
                                        length(diff(full_data$Spread)) - 36)]
yield_for_m3 <- train_data$Yield[seq(38,
                                length(train_data$Yield))]
EL_for_m3 <-  train_data$EL[seq(38, length(train_data$EL))]
